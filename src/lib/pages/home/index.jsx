...
const Home = () => {
  ...
  const [moviePage, setMoviePage] = useState(initialSearchPage || '');
  ...
  useEffect(() => {
    const fetchMovies = async () => {
      ...
      navigate(
        `/?title=${debouncedSearchTitle}&type=${movieType}&year=${movieYear}&page=${moviePage}`,
      );
    };

    fetchMovies();
  }, [debouncedSearchTitle, movieType, movieYear, moviePage, navigate]);
  ...
  return (
    <Box>
          ...
          >
            <Card className="flex flex-col w-full h-full">
              ...
              <CardActions>
                ...
                <Button
                  LinkComponent={Link}
                  to={`/details/${movie.imdbID}/?title=${searchTitle}&type=${movieType}&year=${movieYear}&page=${moviePage}`}
                  size="small"
                >
                  Learn More
                </Button>
      ...
      {movies.length > 0 && (
        <Box className="w-96 mx-auto grid place-items-center mt-10">
          <Stack>
            <Pagination
              defaultPage={1}
              page={parseInt(moviePage)}
              onChange={(event, page) => {
                setMoviePage(page);
